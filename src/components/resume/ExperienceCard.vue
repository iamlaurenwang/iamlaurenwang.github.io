...
<template>
  <div class="flex gap-5">
    ...
    <div class="flex shrink-0 flex-col items-center pt-1.5">
      <div class="h-2.5 w-2.5 shrink-0 rounded-full bg-accent-400 ring-2 ring-accent-100" />
      <div v-if="!isLast" class="mt-2 w-px flex-1 bg-neutral-200" />
    </div>
    ...
    <div class="min-w-0 flex-1 pb-10">
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div class="min-w-0">
          <h3 class="font-serif text-xl text-neutral-800">{{ experience.role }}</h3>
      ...
      <p
        ...
        class="mt-3 font-sans text-sm leading-relaxed text-neutral-600"
      >
      ...
      <div v-if="experience.skills.length" class="mt-4 flex flex-wrap gap-2">
        <PillTag v-for="skill in experience.skills" :key="skill" variant="accent">
          {{ skill }}
        </PillTag>
      </div>
    </div>
