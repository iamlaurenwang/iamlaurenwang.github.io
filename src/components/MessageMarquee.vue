    ...
    >
      <template v-for="pass in (shouldScroll ? 2 : 1)" :key="pass">
        <div
          ...
          :key="`${pass}-${msg.id}`"
          class="w-72 shrink-0 rounded-xl border border-neutral-200 bg-white p-5 shadow-card"
        >
          <div class="mb-3 flex items-center gap-3">
            ...
            <span class="font-sans text-sm font-medium text-neutral-700">{{ msg.author }}</span>
          </div>
          <p class="font-sans text-sm leading-relaxed text-neutral-600">{{ msg.text }}</p>
        </div>
      </template>
    </div>
  </div>
